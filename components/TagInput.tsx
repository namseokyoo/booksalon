    ...
    return (
        <div className={`space-y-3 ${className}`}>
            ...
            <div className="relative">
                ...
                {/* 입력된 태그와 입력 필드 */}
                <div className="flex flex-wrap items-center gap-2 p-2 border border-border rounded-lg bg-surface focus-within:ring-2 focus-within:ring-ring focus-within:border-primary min-h-[42px]">
                    {/* 추가된 태그들 */}
                    {tags.map((tag, index) => (
                        <TagBadge
                            key={`${tag}-${index}`}
                ...
                {showSuggestions && (suggestions.length > 0 || isLoadingSuggestions) && (
                    <div
                        ref={suggestionsRef}
                        className="absolute z-20 w-full mt-1 bg-surface border border-border rounded-lg shadow-lg max-h-48 overflow-y-auto"
                    >
                            ...
                            suggestions.map((suggestion, index) => (
                                <button
                                    ...
                                    className={`w-full px-3 py-2 text-left text-sm hover:bg-primary-50 transition-colors ${
                                        index === selectedSuggestionIndex ? 'bg-primary-50 text-primary-700' : 'text-surface-foreground'
                                    }`}
                                >
                                    #{suggestion}
                                </button>
                    ...
                    <div className="flex flex-wrap gap-2">
                        {availableRecommendedTags.slice(0, 8).map(tag => (
                            <button
                                ...
                                className="px-2 py-1 text-xs rounded-full border border-border bg-muted text-muted-foreground hover:border-primary-300 hover:text-primary-700 hover:bg-primary-50 transition-colors"
                            >
                                #{tag}
                            </button>
                        ))}
