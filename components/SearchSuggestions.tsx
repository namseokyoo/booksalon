        ...
        <div className="max-h-64 overflow-y-auto">
          {suggestions.map((suggestion, index) => (
            <button
              ...
              className="w-full px-4 py-2.5 text-left text-sm text-surface-foreground hover:bg-muted focus:bg-muted focus:outline-none flex items-center gap-2"
            >
              <svg
          ...
          <div className="border-b border-border last:border-b-0">
            ...
            <div>
              {recentSearches.map((term, index) => (
                <div
                  key={`recent-${index}`}
                  className="flex items-center hover:bg-muted group"
                >
                  <button
                    type="button"
                    onClick={() => onSelectSuggestion(term)}
                    className="flex-1 px-4 py-2.5 text-left text-sm text-surface-foreground flex items-center gap-2"
                  >
                    <svg
                      className="w-4 h-4 text-muted-foreground flex-shrink-0"
                      fill="none"
                  ...
                  <button
                    ...
                    className="p-2 mr-2 text-muted-foreground hover:text-foreground opacity-0 group-hover:opacity-100 transition-opacity"
                    aria-label={`${term} 삭제`}
                  >
            ...
            <div>
              {popularSearches.map((term, index) => (
                <button
                  ...
                  className="w-full px-4 py-2.5 text-left text-sm text-surface-foreground hover:bg-muted focus:bg-muted focus:outline-none flex items-center gap-2"
                >
                  <span className="w-5 h-5 flex items-center justify-center text-xs font-medium text-primary bg-primary-50 rounded-full flex-shrink-0">
                    {index + 1}
                  </span>
                  <span className="truncate">{term}</span>
                </button>
