        ...
        <div className="min-h-screen bg-background p-4">
            <div className="max-w-6xl mx-auto">
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 h-[calc(100dvh-200px)]">
                    {/* 사이드바: 모바일에서 채팅방 선택 시 숨김 */}
                    <div className={`lg:col-span-1 bg-surface border border-border rounded-xl p-4 shadow-sm ${selectedChat ? 'hidden md:block' : 'block'}`}>
                        {/* 채팅 목록만 표시 */}
                        <ChatList onSelectChat={handleSelectChat} />
                    </div>

                    {/* 메인 채팅 영역: 모바일에서 채팅방 미선택 시 숨김 */}
                    <div className={`lg:col-span-2 bg-surface border border-border rounded-xl p-4 shadow-sm ${selectedChat ? 'block' : 'hidden md:flex'}`}>
                        {selectedChat ? (
                            <>
                                {/* 뒤로가기 버튼: 모바일에서만 표시 */}
                                <button
                                    onClick={handleCloseChat}
                                    className="md:hidden flex items-center gap-1 text-sm text-primary mb-3 hover:text-primary-700 transition-colors"
